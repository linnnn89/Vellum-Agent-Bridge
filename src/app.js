...
function saveSoon() { state.dirty = true; $('#save-indicator').innerHTML = `<i></i> ${esc(t('topbar.savingLocally'))}`; clearTimeout(saveTimer); saveTimer = setTimeout(save, 500); }
async function save() {
    ...
    try {
        ...
        state.dirty = false;
        $('#save-indicator').innerHTML = `<i></i> ${esc(t('topbar.savedLocally'))}`;
    }
    catch (e) {
        $('#save-indicator').innerHTML = `<i style="background:var(--danger)"></i> ${esc(t('topbar.saveFailed'))}`;
...
function section(title, body, action = '', extra = '', titleIsHtml = false) { return `<section class="inspector-section ${extra}"><div class="section-heading"><span>${titleIsHtml ? title : esc(title)}</span>${action}</div>${body}</section>`; }
...
function renderInspector() {
    ...
    if (!n) {
        html += section(t('inspector.pageSection'), `<div class="fill-row"><div class="hex-field"><input type="color" id="canvas-color" value="${options.canvasColor || (options.theme === 'dark' ? '#1a1a1d' : '#e8e7ec')}" aria-label="Canvas color"><input type="text" value="${options.canvasColor || (options.theme === 'dark' ? '1A1A1D' : 'E8E7EC')}" readonly aria-label="Canvas hex"></div><span class="small-label">100%</span></div><label class="checkbox-row"><input type="checkbox" data-option="grid" ${options.grid ? 'checked' : ''}>${esc(t('settings.grid'))}</label>`, icon('sliders', 14));
        html += section(t('inspector.startWithFrame'), `<div class="property-grid"><button class="wide-button" data-preset="desktop">${esc(t('inspector.presetDesktop'))}</button><button class="wide-button" data-preset="phone">${esc(t('inspector.presetPhone'))}</button><button class="wide-button" data-preset="tablet">${esc(t('inspector.presetTablet'))}</button><button class="wide-button" data-preset="square">${esc(t('inspector.presetSocial'))}</button></div>`);
        html += section(t('inspector.localColorStyles'), doc.data.tokens.colors.map(c => `<button class="style-row full-width" data-insert-color="${safeColor(c.value)}"><span class="style-swatch" style="background:${safeColor(c.value)}"></span><span class="style-info" style="text-align:left">${esc(c.name)}<small>${esc(c.value.toUpperCase())}</small></span>${icon('component', 12)}</button>`).join(''), `<button class="icon-button small" data-action="tokens" title="${esc(t('inspector.editTokensTitle'))}">${icon('sliders', 14)}</button>`);
        ...
        html += section(t('inspector.yourWorkYourDevice'), `<p>${esc(t('inspector.deviceNote'))}</p><button class="wide-button" data-action="saveFile" style="margin-top:13px">${icon('download', 13)} ${esc(t('inspector.savePortableCopy'))}</button>`);
    }
    else {
        const multi = ns.length > 1;
        html += section(`<span class="section-title">${icon(multi ? 'layers' : n.component ? 'component' : n.isInstance ? 'instance' : n.type, 14)}<span class="selection-name">${multi ? esc(t('inspector.selectedLayers', { count: ns.length })) : esc(n.name)}</span></span>`, `<div class="selection-meta">${multi ? esc(t('inspector.sharedProperties')) : n.isInstance ? esc(t('inspector.componentInstance')) : n.component ? esc(t('inspector.mainComponent')) : `${esc(n.type[0].toUpperCase() + n.type.slice(1))} · ${Math.round(n.w)} × ${Math.round(n.h)}`}</div>`, `<button class="icon-button small" data-action="selectionMenu" title="${esc(t('inspector.layerActions'))}">···</button>`, '', true);
        ...
        html += section(t('inspector.exportSection'), `<div class="property-grid" style="margin-bottom:8px"><select id="export-scale" aria-label="Export scale"><option value="1">1×</option><option value="2" selected>2×</option><option value="3">3×</option></select><select id="export-format" aria-label="Export format"><option>PNG</option><option>SVG</option></select></div><button class="export-button" data-action="exportSelection">${icon('download', 13)} ${esc(t('inspector.exportSelection', { name: multi ? t('exportDialog.selection') : (n.name.length > 21 ? n.name.slice(0, 20) + '…' : n.name) }))}</button>`);
        html += section(t('inspector.developer'), `<button class="wide-button" data-action="inspectCSS">${icon('code', 14)} ${esc(t('inspector.inspectCSS'))}</button>`);
    }
    $('#inspector').innerHTML = html;
    lastInspector = performance.now();
}
function renderPrototype(n) { const frames = doc.nodes.filter(n => n.type === 'frame' && !n.parentId); $('#inspector').innerHTML = section(t('inspector.prototypeTitle'), `<p>${esc(t('inspector.prototypeDesc'))}</p>`) + (n ? section(t('inspector.interaction'), `<div class="field-label">${esc(t('inspector.navigateOnClick'))}</div>${selectField('prototypeTarget', n.prototypeTarget || '', [['', t('inspector.noDestination')], ...frames.map(f => [f.id, f.name])])}<p>${esc(t('inspector.transitionInstant'))}</p>`) : section(t('inspector.selectLayer'), `<p>${esc(t('inspector.selectLayerDesc'))}</p>`)) + section(t('inspector.flowPreview'), `<button class="wide-button primary" data-action="present">${icon('play', 14)} ${esc(t('inspector.presentFrames'))}</button><p>${esc(t('inspector.previewNote'))}</p>`); }
...
function modalHeader(title, subtitle = '') { return `<div class="modal-header"><div><h2>${esc(title)}</h2>${subtitle ? `<p style="margin:0">${esc(subtitle)}</p>` : ''}</div><button class="icon-button" data-close-modal title="${esc(t('common.close'))}" aria-label="${esc(t('common.close'))}">${icon('close')}</button></div>`; }
function promptText(title, value, callback) {
    modal(`${modalHeader(title)}<form id="prompt-form"><input class="form-input" id="prompt-value" value="${esc(value)}" required maxlength="200" autocomplete="off"><div class="modal-actions"><button type="button" class="wide-button" data-close-modal>${esc(t('common.cancel'))}</button><button class="wide-button primary" type="submit">${esc(t('common.save'))}</button></div></form>`);
...
function editTokens() {
    modal(`${modalHeader(t('tokensDialog.title'), t('tokensDialog.subtitle'))}<div id="token-editor">${doc.data.tokens.colors.map((c, i) => `<div style="display:flex;gap:8px;margin-bottom:10px"><input type="color" value="${safeColor(c.value)}" data-token-color="${i}" style="width:36px;height:36px"><input class="form-input" value="${esc(c.name)}" data-token-name="${i}"></div>`).join('')}</div><p>${esc(t('tokensDialog.note'))}</p><div class="modal-actions"><button class="wide-button" id="export-tokens">${esc(t('tokensDialog.exportJson'))}</button><button class="wide-button primary" id="save-tokens">${esc(t('tokensDialog.apply'))}</button></div>`);
    $('#save-tokens').onclick = () => {
...
<div class="stack">
  <div style="display:flex;justify-content:space-between;align-items:center;background:var(--surface2);padding:10px 14px;border-radius:8px;border:1px solid var(--border);margin-bottom:6px">
    <div>
      <strong style="font-size:13px;display:block">${esc(t('settings.language'))}</strong>
  ...
  <label class="checkbox-row"><input type="checkbox" id="settings-theme" ${options.theme === 'light' ? 'checked' : ''}>${esc(t('settings.themeLight'))}</label>
  <label class="checkbox-row"><input type="checkbox" data-option="grid" ${options.grid ? 'checked' : ''}>${esc(t('settings.grid'))}</label>
  <label class="checkbox-row"><input type="checkbox" data-option="snap" ${options.snap ? 'checked' : ''}>${esc(t('settings.snap'))}</label>
  <label class="checkbox-row"><input type="checkbox" data-option="rulers" ${options.rulers ? 'checked' : ''}>${esc(t('settings.rulers'))}</label>
</div>
<h3>${esc(t('settings.rendering'))}</h3>
<pre class="token-code">${esc(t('settings.backend', { backend: renderer.backend }))}
${esc(t('settings.visibleLayers', { count: renderer.visibleCount }))}
${esc(t('settings.instances', { count: renderer.instanceCount }))}
${esc(t('settings.sceneDrawCalls', { count: renderer.drawCalls }))}
${esc(t('settings.submissionTime', { ms: renderer.cpuMs.toFixed(2) }))}
${esc(t('settings.devicePixelRatio', { dpr: renderer.dpr }))}
${esc(t('settings.storage', { mode: storageMode }))}${renderer.backend !== 'WebGPU' ? `\n${esc(t('settings.webgpuStatus', { status: renderer.gpuError || 'Unavailable' }))}` : ''}</pre>
<p>${esc(t('settings.renderNote'))}</p>`);
...
function openExport() { modal(`${modalHeader(t('exportDialog.title'), t('exportDialog.subtitle'))}<button class="wide-button primary" id="modal-save-file" style="height:43px">${icon('download')} ${esc(t('exportDialog.downloadDoc'))}</button><p>${esc(t('exportDialog.downloadNote'))}</p><h3>${esc(t('exportDialog.exportHeading', { target: state.selection.size ? t('exportDialog.selection') : t('exportDialog.currentPage') }))}</h3><div class="property-grid"><button class="wide-button" id="modal-png">${esc(t('exportDialog.pngOption'))}</button><button class="wide-button" id="modal-svg">${esc(t('exportDialog.svgOption'))}</button></div><h3>${esc(t('exportDialog.alreadyHave'))}</h3><button class="wide-button" id="modal-open-file">${icon('upload')} ${esc(t('exportDialog.openDoc'))}</button><p>${esc(t('exportDialog.jsonNote'))}</p>`); $('#modal-save-file').onclick = saveFile; $('#modal-png').onclick = () => doExport('PNG', 2); $('#modal-svg').onclick = () => doExport('SVG'); $('#modal-open-file').onclick = () => { $('#file-input').click(); closeModal(); }; }
function newFile() { modal(`${modalHeader(t('newFileDialog.title'))}<p>${esc(t('newFileDialog.desc'))}</p><div class="modal-actions"><button class="wide-button" data-close-modal>${esc(t('common.cancel'))}</button><button class="wide-button" id="backup-new">${esc(t('newFileDialog.backup'))}</button><button class="wide-button primary" id="confirm-new">${esc(t('newFileDialog.confirm'))}</button></div>`); $('#backup-new').onclick = saveFile; $('#confirm-new').onclick = () => { closeModal(); transaction('New document', () => { const p = { id: uid(), name: 'Page 1', nodes: [] }; doc.data.name = 'Untitled design'; doc.data.pages = [p]; doc.data.pageId = p.id; doc.refresh(); state.selection.clear(); state.pageViews.clear(); fit(); }); }; }
function stressTest() {
...
function commandPalette() {
    ...
    modal(`<input class="command-search" id="command-search" placeholder="${esc(t('commands.searchPlaceholder'))}" aria-label="${esc(t('commands.searchAria'))}" autocomplete="off"><div class="command-results" id="command-results"></div><p style="font-size:10px;margin-bottom:0">${esc(t('commands.navigateHint'))}</p>`);
    ...
    const render = () => {
        ...
        $('#command-results').innerHTML = filtered.map(([label, action], i) => `<button class="menu-item ${i === selectedIndex ? 'focused' : ''}" data-command="${action}"><span>${esc(label)}</span><span class="shortcut">↵</span></button>`).join('') || `<div class="empty-state">${esc(t('commands.noMatching'))}</div>`;
    };
